...
function MoviesCardList({
  ...
  const [addedCardsAmmount, setAddedCardsAmmount] = useState(
    XL_ON_MORE_CARDS_AMMOUNT
  );
  ...
  function handleClickBtnMore() {
    setInitialQuantity(initialQuantity + addedCardsAmmount);
  }

  useEffect(() => {
    if (width < S_WIDTH) {
      setInitialQuantity(S_CARDS_AMMOUNT);
      setAddedCardsAmmount(S_ON_MORE_CARDS_AMMOUNT);
    } else if (width < M_WIDTH) {
      setInitialQuantity(M_CARDS_AMMOUNT);
      setAddedCardsAmmount(M_ON_MORE_CARDS_AMMOUNT);
    } else if (width < L_WIDTH) {
      setInitialQuantity(L_CARDS_AMMOUNT);
      setAddedCardsAmmount(L_ON_MORE_CARDS_AMMOUNT);
    } else {
      setInitialQuantity(XL_CARDS_AMMOUNT);
      setAddedCardsAmmount(XL_ON_MORE_CARDS_AMMOUNT);
    }
  }, [width]);
